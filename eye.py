...
while True:
    ...
    if len(faces) == 0:
        ...
        if timer_start is None:
            ...
            # If the timer has been running for more than 0.5 seconds (excluding camera delay), set the GPIO output to high
        ...
        for (x, y, w, h) in faces:
            ...
            if len(eyes)<=1:
                continous +=1
                print(continous)
                if continous >=6: #If 6 or more frames are missed in a row
                    if timer_start is None:
                        timer_start = time.time()
                    if time.time() - timer_start >= 0:
                        GPIO.output(14, GPIO.HIGH)
                else:
                    if timer_start is None:
                        timer_start = time.time()
